...
const debug = require('debug')('bleno:gatt');
          ...
          if (data.length !== 2) {
            ...
            if (value & 0x0003) {
              ...
              if (handleAttribute.emit) {
                handleAttribute.emit('subscribe', this._mtu - 3, updateValueCallback, callback);
              } else {
                callback(ATT_ECODE_REQ_NOT_SUPP)
              }
            } else {
              if (handleAttribute.emit) {
                handleAttribute.emit('unsubscribe', callback);
              } else {
                callback(ATT_ECODE_REQ_NOT_SUPP)
              }
            }
